...
def create_float_tensor(shape, on_gpu, fill_value=None):
    if on_gpu:
        res = torch.cuda.FloatTensor(*shape)
...
def warp_image_translation(ref_image, flo_image, param, mode='nearest', bg_value=0.0, inv=False):
    ...
    # Ensure bg_value is properly handled
    if isinstance(bg_value, (int, float)):  # If bg_value is a scalar
        bg_value = np.full(flo_image.shape[2], bg_value)  # Extend it to match the number of channels
    
    if flo_image.ndim == 3:  # Multi-channel image
        for i in range(flo_image.shape[2]):  # Loop through each channel
            bg_val_i = np.array(bg_value[i])
            t.warp(flo_image[:, :, i], flo_image_out[:, :, i], in_spacing=np.ones(2,), out_spacing=np.ones(2,), mode=mode, bg_value=bg_val_i)
    else:  # Single-channel image
        t.warp(flo_image, flo_image_out, in_spacing=np.ones(2,), out_spacing=np.ones(2,), mode=mode, bg_value=bg_value[0] if isinstance(bg_value, (list, np.ndarray)) else bg_value)
...
### Usage example
def align(ref, tar):
    h, w, c = ref.shape[0], ref.shape[1], ref.shape[2]
    Q = 16  # Number of quantization levels per channel, 8 works before
    ref = ref.copy().astype(np.float32)
    tar = tar.copy().astype(np.float32)

    # Apply K-means on multi-channel data
    ref_q = image2cat_kmeans(ref, Q)
    tar_q = image2cat_kmeans(tar, Q)
...
# Modified image2cat_kmeans for multi-channel
def image2cat_kmeans(I, k, batch_size=100, max_iter=1000, random_seed=1000):
    total_shape = I.shape
    channels = I.shape[2] if I.ndim == 3 else 1
    if channels > 1:
    ...
    if k == 1:
        return np.zeros(I_lin.shape[0], dtype='int').reshape(I.shape[:2])
    ...
    return I_res.reshape(I.shape[:2])